  ...
  return (
    <View>
      ...
      <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.card }]}>
        ...
        <Text style={[styles.totalValue, { color: colors.text }]}>{formatCurrency(summary.totalValue)}</Text>
        ...
        <View style={styles.pnlRow}>
          ...
          <View style={[styles.pctBadge, { backgroundColor: isPositive ? colors.positiveBg : colors.negativeBg }]}>
            <Text style={[styles.pctText, { color: isPositive ? colors.positive : colors.negative }]}>
              {formatPercentage(summary.totalReturnPct)}
            </Text>
          </View>
          <Text style={[styles.alltime, { color: colors.textMuted }]}>{t('portfolio.alltime')}</Text>
        </View>
      ...
      {/* Cash row */}
      <View style={[styles.cashRow, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.card }]}>
        <Text style={[styles.cashLabel, { color: colors.textSecondary }]}>{t('portfolio.availableTrade')}</Text>
...
const styles = StyleSheet.create({
  title: { fontSize: 28, fontWeight: '700', letterSpacing: -0.84, marginTop: 6, marginBottom: 18 },
  card: { borderWidth: 1, padding: 20, paddingBottom: 8 },
  label: { fontSize: 13, fontWeight: '500' },
  totalValue: { fontSize: 34, fontWeight: '700', letterSpacing: -0.68, marginTop: 3 },
  ...
  cashRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, padding: 14, paddingHorizontal: 18, marginTop: 12 },
